...
| **RFI 작성** — 필요 데이터 목록 (SKU 마스터 필드 + 마스터/트랜잭션 데이터 구분 포함) 정리해 창신 측 전달 | 🔥 | 미팅 핵심 본인 액션. 가능한 빨리 |
| **ERP 액세스 재확인** — 엄영태 팀장 계정으로 발급된 것으로 클라 인지. 본인은 못 받음. 회장님 통해 재전달 약속됨 | ❓ | 클라 측 답변 대기 + 본인이 엄영태 팀장에게 직접 확인 |
| **스케줄 모듈 방향 구체화** — "기수주 반영 예상 납기" entity / 서비스 골격 작성 | 🔥 | M1, M2 entity. RFI 와 병행 가능 |

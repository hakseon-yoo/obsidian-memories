...
| 항목 | 상태 | 출처 |
|------|------|------|
| Module 1 · 수주 예측·확정 모듈 골격 | 🔥 | [[AX-2 지능형 스케줄러/02 - Module 1 · 수주 예측·확정]] |
| Module 2 · 스케줄링 모듈 골격 | 🔥 | [[AX-2 지능형 스케줄러/03 - Module 2 · 스케줄링]] |
| Module 3 · 생산 관제 (Phase 3) | 📅 | [[AX-2 지능형 스케줄러/04 - Module 3 · 생산 관제]] |
| Module 4 · 물류 (Phase 3) | 📅 | [[AX-2 지능형 스케줄러/05 - Module 4 · 물류]] |
| (부속 영역) 구매 · 정산 (모듈 미확정) | 📅 | [[AX-2 지능형 스케줄러/06 - 부속 영역 · 구매 · 정산]] |
...
| **5월 말 vs 6월 말 타임라인 정합성 점검** — 본인 P0(6월말) ↔ 회의(5월말 1차) | ❌ | [[AX-2 지능형 스케줄러/11 - 프로젝트 착수 질의 리스트#0. P0 빠른 참조 (착수 전 결정 필수)\|#64]] |
...
| ~~수주 시나리오 입력 화면 필드 설계~~ → **HTML 1차 산출물 채택 + 보완 4항목 적용 (2026-05-07)** | ✅ | 동료 개발자 HTML(`changshin/documents/창신-ax2.html`) 의 `mNewOrder` 모달에 시나리오 enum, 신제품 SKU 라벨 동적 변경, 금형 메타 입력 영역, 컨텍스트 안내 패치. 와이어프레임 문서는 [[AX-2 지능형 스케줄러/02a - 수주 입력 화면 와이어프레임 (draft)\|보완 항목 트래커]] 로 재포지셔닝 |
| ~~수주 확정 후 프로세스 온라인 미팅 일정 조율~~ → **2026-05-07 15:00 확정** (카톡, 서주현 팀장) | ✅ | 2026-05-07 |
| **수주 확정 후 프로세스 온라인 미팅 진행** — 2026-05-07 (오늘) 15:00 · 서주현 팀장 (모니터 딜로이트) | 🔥 | prep 문서 + 질문 리스트 준비 완료: [[Meetings/2026-05-07 - 수주 확정 후 프로세스 설명]] |
...
> 마스터: [[AX-2 지능형 스케줄러/11 - 프로젝트 착수 질의 리스트]]

### B-1. ERP 연동 (3건)

| 항목 | 상태 | 출처 |
|------|------|------|
| 현재 ERP 정체 (제품·버전·벤더) | ❓ | [[AX-2 지능형 스케줄러/11 - 프로젝트 착수 질의 리스트#2. 데이터 · 외부 시스템\|#26]] |
| ERP 연동 방식 가능성 (REST/DB/배치/큐) | ❓ | [[AX-2 지능형 스케줄러/11 - 프로젝트 착수 질의 리스트#2. 데이터 · 외부 시스템\|#27]] |
| ERP 데이터 실시간성 (폴링 주기·푸시 가능?) | ❓ | [[AX-2 지능형 스케줄러/11 - 프로젝트 착수 질의 리스트#2. 데이터 · 외부 시스템\|#28]] |

### C-1. 사용자 · 인증 (3건)

| 항목 | 상태 | 출처 |
|------|------|------|
| 사용자 그룹 정체 (창신/외주처/ODM/고객사) | ❓ | [[AX-2 지능형 스케줄러/11 - 프로젝트 착수 질의 리스트#3. 사용자 · 인증 · 권한\|#34]] |
| 외주처 직원 계정 발급 방식 | ❓ | [[AX-2 지능형 스케줄러/11 - 프로젝트 착수 질의 리스트#3. 사용자 · 인증 · 권한\|#35]] |
| 창신 사내 SSO 존재 (OIDC/SAML) | ❓ | [[AX-2 지능형 스케줄러/11 - 프로젝트 착수 질의 리스트#3. 사용자 · 인증 · 권한\|#36]] |

### A-1. AX-2 도메인 · 비즈니스 (P0 9건)

| 항목 | 상태 | 출처 |
|------|------|------|
| 납기일 예측의 타겟 납기일 세팅 로직 | ❓ | [[AX-2 지능형 스케줄러/11 - 프로젝트 착수 질의 리스트#1-1. Module 1 — 수주 예측·확정\|#1]] |
| 공정별 "며칠 전 컨펌 요청" D-day 기준 | ❓ | [[AX-2 지능형 스케줄러/11 - 프로젝트 착수 질의 리스트#1-2. Module 2 · 스케줄링 + Module 3 · 생산 관제\|#6]] |
| WIP 품질 판정 워크플로우 (이중 컨펌 / 사진 필수?) | ❓ | [[AX-2 지능형 스케줄러/11 - 프로젝트 착수 질의 리스트#1-2. Module 2 · 스케줄링 + Module 3 · 생산 관제\|#7]] |
| 외주처 PDA 활용 가능 여부 + 대안 | ❓ | [[AX-2 지능형 스케줄러/11 - 프로젝트 착수 질의 리스트#1-3. Module 4 — 물류\|#11]] |
| ODM 창고 미예약 사고 통계 | ❓ | [[AX-2 지능형 스케줄러/11 - 프로젝트 착수 질의 리스트#1-3. Module 4 — 물류\|#12]] |
| 재생산 공제 로직 상세 | ❓ | [[AX-2 지능형 스케줄러/11 - 프로젝트 착수 질의 리스트#1-4. 부속 영역 — 구매 · 정산\|#17]] |
| 품질 귀책 판단 주체·표준 절차 | ❓ | [[AX-2 지능형 스케줄러/11 - 프로젝트 착수 질의 리스트#1-4. 부속 영역 — 구매 · 정산\|#18]] |
| 입고 시 불량/수량 부족 ERP 미입력 흐름 | ❓ | [[AX-2 지능형 스케줄러/11 - 프로젝트 착수 질의 리스트#1-5. 공통 · 전반\|#22]] |
| 금형 샷 수 관리 위치·계측 방법 | ❓ | [[AX-2 지능형 스케줄러/11 - 프로젝트 착수 질의 리스트#1-5. 공통 · 전반\|#23]] |
| **(SKU 마스터 클렌징 책임 부서·완료 시점)** — 1차 완료, 후속 41건 정리 책임만 미정 | ⏳ | [[AX-2 지능형 스케줄러/11 - 프로젝트 착수 질의 리스트#1-1. Module 1 — 수주 예측·확정\|#2]] |

### D-1. 운영 · 규모 · SLA · 보안 (P0 4건)

| 항목 | 상태 | 출처 |
|------|------|------|
| 예상 동시 사용자 수 (피크/평균) | ❓ | [[AX-2 지능형 스케줄러/11 - 프로젝트 착수 질의 리스트#4. 운영 · 규모 · SLA\|#41]] |
| 운영 시간대 (24/7 vs 평일 09-18, 야간 배치) | ❓ | [[AX-2 지능형 스케줄러/11 - 프로젝트 착수 질의 리스트#4. 운영 · 규모 · SLA\|#42]] |
| 데이터 거주 요구 (한국/일본 리전?) | ❓ | [[AX-2 지능형 스케줄러/11 - 프로젝트 착수 질의 리스트#5. 보안 · 컴플라이언스\|#48]] |
| 개인정보 취급 항목 식별 | ❓ | [[AX-2 지능형 스케줄러/11 - 프로젝트 착수 질의 리스트#5. 보안 · 컴플라이언스\|#49]] |

### D-2. 인프라 · 도메인 (임시 결정, 영구화 필요)

| 항목 | 상태 | 출처 |
|------|------|------|
| AWS 계정 창신 보유 여부 | ⏳ | [[AX-2 지능형 스케줄러/11 - 프로젝트 착수 질의 리스트#6. 인프라 · 배포 환경\|#54]] · [[Infrastructure/31 - Decision Log#D-012]] |
| 도메인 보유 여부 (`changshin.io` 등) | ⏳ | [[AX-2 지능형 스케줄러/11 - 프로젝트 착수 질의 리스트#6. 인프라 · 배포 환경\|#55]] · [[Infrastructure/31 - Decision Log#D-015]] |
| 클라이언트 도메인 확정 시 인증서 + host 일괄 교체 | 📅 | [[Infrastructure/31 - Decision Log#미결정 · 논의 필요]] |
| 현장 파일럿 대상 (어느 외주처·SKU?) | ❓ | [[AX-2 지능형 스케줄러/11 - 프로젝트 착수 질의 리스트#8. 일정 · 우선순위 · 범위\|#66]] |

### ⛔ 본인 책임 외 (AX1·AX3 담당자 위임)

- AX1 도메인 영역, AX3 도메인 영역, AX1·AX3 착수 시기 — [[AX-2 지능형 스케줄러/11 - 프로젝트 착수 질의 리스트#7. AX 시리즈 도메인 명확화|7번 섹션]]
...
| ML 기반 납기 예측 도입 | 📅 | [[AX-2 지능형 스케줄러/08 - 제안 로드맵]] |
| 금형 메인터넌스 사이클 자동화 | 📅 | [[AX-2 지능형 스케줄러/07 - 공통 · 보조 영역]] |
...
| 일자 | 항목 |
|------|------|
| 2026-05-07 | **HTML 1차 산출물 채택 + 클라 답변 4항목 보완** — 동료 개발자 작성 HTML(`changshin/documents/창신-ax2.html`)을 화면 마스터로 정의. `mNewOrder` 모달에 시나리오 enum (existing/new/reproduction), 신제품 SKU 라벨 동적 변경, 금형 메타 입력 영역, 컨텍스트 안내 패치. 미팅 시연용 |
| 2026-05-07 | **15:00 미팅 사전 준비 완료** — 수주 입력 화면 1차 와이어프레임 ([[AX-2 지능형 스케줄러/02a - 수주 입력 화면 와이어프레임 (draft)]]) + 미팅 prep 문서 + 질문 리스트 (Q1~Q7) 작성 ([[Meetings/2026-05-07 - 수주 확정 후 프로세스 설명]]) |
| 2026-05-07 | **수주 확정 후 프로세스 미팅 일정 확정** — 2026-05-07 15:00 (서주현 팀장 · 모니터 딜로이트). 카톡 후속 합의 |

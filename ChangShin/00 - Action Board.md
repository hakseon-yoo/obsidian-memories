...
| ~~수주 확정 후 프로세스 온라인 미팅 진행~~ → **2026-05-07 15:00 진행 완료** | ✅ | 회의록: [[Meetings/2026-05-07 - 수주 확정 후 프로세스 설명]] |
| **🚨 ERP 액세스 확보 (모든 후속 작업 차단)** — 엄영태 팀장(팀스파르타 PM) 계정으로 발급된 것으로 클라 측 인지. 본인은 못 받음. **본인이 엄영태 팀장에게 직접 확인** + 클라 측 라인 재점검 요청. ([[Infrastructure/02 - Stakeholder Map]] 참조) | 🔥 | **우선순위 1번**. RFI/entity 골격 등 후속 작업의 선결 조건 |
| ~~RFI 작성~~ → **보류 (ERP 액세스 후)** — ERP 자체를 봐야 마스터/필드 구분 가능. 액세스 전엔 추상 카테고리 수준만 가능 | 📅 | Blocked by ERP 액세스. 액세스 후 진행 |
| **ERP 액세스 요청서 작성** — "이런 데이터를 봐야 RFI 가능, 읽기 권한 필요" 명시적 1페이지 요청서 (RFI 대신 지금 가능한 산출물) | 🔥 | 클라에 보낼 단기 산출물. RFI 보류의 근거 문서 |
| **스케줄 모듈 방향 구체화** — "기수주 반영 예상 납기" entity / 서비스 골격 작성 | ⏳ | ERP 액세스 후 본격 진행. 그 전에는 추상 골격까지만 |
| **ERP 공정별 납기 이력 데이터 존재 여부 확인** — 1년치 = 대용량 가능성 | ❓ | 클라 측 (서주현 팀장) 답변 대기 |
| **외주처 알림 방식 (건별 vs 배치) 정책 결정** | 📅 | 후속 미팅 |
| **후속 미팅 잡기** — 부속(구매·정산), M3 생산관제, M4 물류 상세 설계 | 📅 | 미정. 일정 조율 필요 |
| **기제품 부자재·공정별 평균 소요 변수 인터뷰 결과 수령** — 서주현 팀장이 생산관리팀장 인터뷰 후 공유 예정 (2026-04-30 약속, 2026-05-07 재확인) | ❓ | Module 1 알고리즘 본체 구현 트리거 |
...
### B-1. ERP 연동 (3건 — 2026-05-07 부분 답변 받음)

| 항목 | 상태 | 출처 |
|------|------|------|
| 현재 ERP 정체 (제품·버전·벤더) | ❓ | [[AX-2 지능형 스케줄러/11 - 프로젝트 착수 질의 리스트#2. 데이터 · 외부 시스템\|#26]] · ERP 액세스 후 직접 확인 |
| ERP 연동 방식 가능성 (REST/DB/배치/큐) | ⏳ | [[AX-2 지능형 스케줄러/11 - 프로젝트 착수 질의 리스트#2. 데이터 · 외부 시스템\|#27]] · "ERP 자체 연동 가능" 답변. 구체 스펙은 RFI 후 |
| ERP 데이터 실시간성 (폴링 주기·푸시 가능?) | ✅ | [[AX-2 지능형 스케줄러/11 - 프로젝트 착수 질의 리스트#2. 데이터 · 외부 시스템\|#28]] · "실시간 어렵다 = 생산 라인 → ERP 자동 입력 X 의 의미. ERP 자체 송수신은 폴링/배치 가능" |
...
| 항목 | 상태 | 출처 |
|------|------|------|
| 납기일 예측의 타겟 납기일 세팅 로직 | ⏳ | [[AX-2 지능형 스케줄러/11 - 프로젝트 착수 질의 리스트#1-1. Module 1 — 수주 예측·확정\|#1]] · 2026-05-07 추가 답변: 현재 약 40일 + 생산관리 팀장 경험 + 매일 엑셀 수기. **시스템 알고리즘 = ERP 기수주 데이터 기반 점진 추가** 로 합의 |
...
| 외주처 PDA 활용 **불가 확정** (PPT Slide 3 + 미팅 재확인) — 입고 데이터 수집 대안만 미정 | ⏳ | [[AX-2 지능형 스케줄러/11 - 프로젝트 착수 질의 리스트#1-3. Module 4 — 물류\|#11]] |
...
| 일자 | 항목 |
|------|------|
| 2026-05-07 | **15:00 미팅 진행 + 회의록 작성** — (1) 예상 납기 = ERP 기수주 데이터 기반 점진 추가 합의, (2) SKU-BOM-외주처 매핑 ERP 보유 확인, (3) ERP 자체 연동 가능, (4) 내부 생산 라인 = 외주처 동일 로직, (5) 공정 5단계 (사출/코팅/인쇄/박/조립), (6) SKU = 공병 단위. 후속 미팅 합의 ([[Meetings/2026-05-07 - 수주 확정 후 프로세스 설명]]) |
| 2026-05-07 | **Vault 모듈 정합 (PPT 4개 모듈)** — M1 수주예측확정 / M2 스케줄링 / M3 생산관제(신규) / M4 물류 + 부속 영역(구매·정산) 으로 재정렬. 14개 파일 mv/sed/edit 일괄 처리 |
